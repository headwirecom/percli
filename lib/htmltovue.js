...
function htmltovue(name, container, compile, deploy, dialog, model, vue, category) {
    ...
    if(fs.existsSync(blockGenerator) || fs.existsSync(blockGenerator+'.txt')) {
        ...
        if (!isobject) {
            ...
            //if (convertVue && templateVue.valueOf() !== newTemplateVue.valueOf()) {
            if (convertVue) {
            ...
            if (sampleFiles.length > 1) {
                for (index in sampleFiles) {
                    let sampleJson = JSON.parse(fs.readFileSync(path.join(componentPath, sampleFiles[index])).toString())
                    functions.createContentXML(sampleJson, uiAppsComponentPath, true)
                }
            } else if (sampleFiles.length === 1) {
                let sampleJson = JSON.parse(fs.readFileSync(path.join(componentPath, sampleFiles[0])).toString())
                functions.createContentXML(sampleJson, uiAppsComponentPath, false)
            }
        ...
        $RefParser.dereference(def, function(err, schema) {
          if (err) {
            ...
            var oldDialog = fs.readFileSync(path.join(uiAppsComponentPath, 'dialog.json')).toString()
            ...
            def.componentPath = projectName + '/'+category+'/' + componentName
