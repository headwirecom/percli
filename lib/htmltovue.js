        ...
        if (!isobject) {
            ...
            fs.readdirSync(componentPath).forEach(file => {
              if (file.includes('sample')) {
                sampleFiles[int] = JSON.parse(fs.readFileSync(path.join(componentPath, file)).toString())
                int = int + 1
              }
            })
            if (sampleFiles.length > 1) {
                functions.createVersionedContentXML(sampleFiles, uiAppsComponentPath)
            } else if (sampleFiles.length === 1) {
                functions.createContentXML(sampleFiles[0], uiAppsComponentPath)
            }
