...
module.exports = {
    ...
    */
    ...
    /**
        * Downloads a list of files and writes them to the output folder
        * @param {array} packages - the packages
        * @return {download} the download
    */
    downloadFiles(packages, sling){
        return axios.all(packages.map(package => {
            if(!package.startsWith('#')) {
                return download(package, package.substring(package.lastIndexOf('/')+1))
            }
        }))
    }
}
