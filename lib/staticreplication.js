...
function findAllFilesRecursively(rootPath) {
    ...
    fs.readdirSync(rootPath).forEach(fileName => {
        ...
        if (fs.lstatSync(fullPath).isDirectory()) {
            if (fullPath.substring(length - RENDITIONS.length, length) === RENDITIONS) {
        ...
        } else if (fullPath.substring(length - THUMBNAIL.length, length) === THUMBNAIL) {
...
function printInfo() {
    ...
    console.log(`# of files chamged on the server: ${checked.changed.length}`)
