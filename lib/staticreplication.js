...
function grabChildrenNodesInfo(node) {
    const children = (node?.hasChildren && node?.children) || []
    return Promise.all(children.map(child => grabNodesInfo(child.path))).then(fullChildren => {
        for (let i = 0; i < children.length; i++) {
            children[i].children = fullChildren[i].children
        }
