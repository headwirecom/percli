...
function checkConsistencyFromStagePOV() {
    grabNodesInfo(tenantPath)
        .then(grabChildrenNodesInfo)
        .then(extractRenderableResourcesFromNodesTree)
        .then(array => array.filter(x => !filesLive.check.includes(rootDirPath + x)))
        .then(printStagePOVConsistencyInfo)
}

function grabNodesInfo(path) {
...
async function grabChildrenNodesInfo(node) {
    const children = (node?.hasChildren && node?.children) || []
    const fullChildren = await Promise.all(children.map(child => grabNodesInfo(child.path)))
    for (let i = 0; i < children.length; i++) {
        const child = children[i]
        child.children = fullChildren[i].children || []
        await grabChildrenNodesInfo(child)
    }

    return node
}

function extractRenderableResourcesFromNodesTree(root) {
    let result = []
    const { path, resourceType, children } = root
    if (resourceType === 'per:Asset') {
        result.push(path)
    } else if (resourceType === 'per:Page') {
        result.push(`${path}.html`)
        result.push(`${path}.data.json`)
    }

    children.map(extractRenderableResourcesFromNodesTree).forEach((e, i, a) => {
...
function printStagePOVConsistencyInfo(missingFiles) {
    ...
    console.log(`# of staged pages and assets not found in the live folder: ${resources.length}`)
    i = 1
