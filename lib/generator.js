        ...
        if (propName !== 'children') {
            ...
            if(props['x-source'] == 'inject') {
                ...
                if(props['x-form-type'] === ('collection')) {
                    if (Object.keys(props['properties']).length > 1) {
...
function genGetters(properties, customGetters) {
    ...
    for(var prop in properties) {
        ...
        if (propName !== 'children' && !customGetters.includes('public String get' + getterName + '()')) {
            var props = properties[propName]
            if(props['x-source'] == 'inject') {
                inject += '\t/* '+JSON.stringify(props)+' */\n'
                if(props['x-form-type'] === ('collection')) {
...
function gen(generator) {
    ...
    var customGetters = dstCurrent.match(/\/\/GEN\[:CUSTOMGETTERS[\s\S]*?\/\/GEN\]/g)[0]
