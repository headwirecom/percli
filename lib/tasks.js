...
*/
function startSling(type, debug, debugPort, suspended) {
    ...
    require('./available.js')(['java'], function(success, tech) {
        if(success) {
            ...
            if(debug) {
                ...
                suspend = suspended ? "y" : "n"
                if(typeof debugPort === 'undefined' || debugPort === "") {
                    debugPort = "30303"
                }
                args.unshift("-agentlib:jdwp=transport=dt_socket,address=" + debugPort + ",server=y,suspend=" + suspend)
                args.unshift("-Xdebug")
            }
...
module.exports = {
    ...
    */
    startPeregrine(type,debug, debugPort, suspended) {
        return new Promise((resolve, reject) => checkIfRunning().then(() => {
            console.log('sling is not running')
            let started = true;
            try {
                startSling(type, debug, debugPort, suspended)
            } catch(error) {
                started = false;
                reject(error);
            }
