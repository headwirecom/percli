...
const f = {
    ...
    createContentXML(sampleFile, componentPath) {
    ...
    createVersionedContentXML(sampleFiles, componentPath, versioned) {
        var title = sampleFiles[0].title
        var group = sampleFiles[0].group
        var version = 1
        var fs = require('fs')
        var path  = require('path')
        var builder = require('xmlbuilder')
        ...
        for (index in sampleFiles) {
            if (sampleFiles[index].default) {
                title = sampleFiles[index].title
                group = sampleFiles[index].group
            }
            f['createVersionedContent'](content, sampleFiles[index], version)
            version = version + 1
        }
    ...
    createVersionedContent(xml, sampleFile, version) {
        var title = sampleFile.title
        var version = xml.ele('v'+version).att('jcr:primaryType','nt:unstructured')
