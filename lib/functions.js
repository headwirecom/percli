...
const f = {
	...
	addPerBlock: function($) {
        $.prepend('<per-block :model="model">')
        $.append('</per-block>')
    },
    ...
    integrateReferences(from) {
        ...
        for(var key in from.properties) {
            ...
            if(field['$ref']) {
                ...
                var refVal = field['$ref'].split('#')
                const componentName = refVal[0]
                const fs    = require('fs-extra')
                const path  = require('path')
                let projectPath = process.cwd()
                let componentPath = path.join(projectPath, './fragments/'+componentName)
                console.log(JSON.parse(fs.readFileSync(path.join(componentPath, 'model.json')).toString()))
            }
        }
        return from
    }
