...
const f = {
    ...
    createContentXML(from, componentPath, versioned) {
        var title = from.title
        var fs = require('fs')
        ...
        if (versioned) {
            f['createVersionedContent'](xml, from, componentPath)
        } else {
            f['createContent'](xml, from)
        }
        ...
        fs.writeFileSync(path.join(componentPath, '.content.xml'), xmlString)

    },

    createContent(xml, from) {
        var title = from.title
        var group = from.group
        var content = xml.ele('jcr:content')
        content.att('jcr:primaryType','nt:unstructured')
        content.att('jcr:title',title)
        content.att('group',group)
        for(var key in from.model) {
            var value = from.model[key]
    ...
    createVersionedContent(xml, from, componentPath) {
        var fs = require('fs')
        ...
        var version = content.ele('v1').att('jcr:primaryType','nt:unstructured')
        var versionContent = version.ele('jcr:content').att('jcr:primaryType','nt:unstructured')
        versionContent.att('jcr:title',title)
        versionContent.att('group',group)
        //read in current .content.xml
       /* var parser = new xml2js.Parser();
        fs.readFile(componentPath + '/.content.xml', function(err, data) {
